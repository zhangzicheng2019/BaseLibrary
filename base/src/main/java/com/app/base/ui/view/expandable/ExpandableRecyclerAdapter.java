package com.app.base.ui.view.expandable;
...
public abstract class ExpandableRecyclerAdapter<P extends Parent<C>, C, PVH extends ParentViewHolder, CVH extends ChildViewHolder>
    ...
    public ExpandableRecyclerAdapter() {
        ...
        mExpansionStateMap = new HashMap<>();
    }
    ...
    public int getHeaderViewCount() {
        return vHeader == null ? 0 : 1;
    }

    private int getRealPosition(RecyclerView.ViewHolder holder) {
        int position = holder.getLayoutPosition();
        return vHeader == null ? position : position - 1;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int flatPosition) {
        ...
        if(getItemViewType(flatPosition) == TYPE_HEADER){
            return;
        }
        flatPosition = getRealPosition(holder);
        ...
        Log.e("", "flatPosition:" + flatPosition);
    ...
    public int getItemCount() {
       /* if(mFlatItemList.size() == 0){
            return mFlatItemList.size();
        }*/
        return mFlatItemList.size() + getHeaderViewCount();
    }
    ...
    public int getItemViewType(int flatPosition) {
        if(flatPosition == 0 && vHeader != null){
            return TYPE_HEADER;
        }
        if(vHeader != null){
            flatPosition -= 1;
    ...
    private ParentViewHolder.ParentViewHolderExpandCollapseListener mParentViewHolderExpandCollapseListener
            = new ParentViewHolder.ParentViewHolderExpandCollapseListener() {
    ...
    private void updateExpandedParent(@NonNull ExpandableWrapper<P, C> parentWrapper, int flatParentPosition, boolean expansionTriggeredByListItemClick) {
        ...
        if (wrappedChildList != null) {
            ...
            notifyItemRangeInserted(flatParentPosition + 1 + getHeaderViewCount(), childCount);
        }
    ...
    private void updateCollapsedParent(@NonNull ExpandableWrapper<P, C> parentWrapper, int flatParentPosition, boolean collapseTriggeredByListItemClick) {
        ...
        if (wrappedChildList != null) {
            ...
            notifyItemRangeRemoved(flatParentPosition + 1 + getHeaderViewCount(), childCount);
        }
